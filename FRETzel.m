...
% Last Modified by GUIDE v2.5 23-Sep-2019 08:50:25
...
handles.cellRadius = cellRadius;
handles.maskCellFinal = maskCellFinal;
        ...
        if size(imageData2, 3) > numChannels                                % pre frame and timeseries    
            ...
            for j = 1:numFiles/numChannels+1                                % for the number of frames that there are in total
                for p=1:size(maskCellFinal,3)                               % for the number of cell outlines
                    ...
                    donorInt(p,j)=sum(donorImage(maskCellFinal(:,:,p)));     % donor intensity of cell
                    FRETInt(p,j)=sum(FRETimage(maskCellFinal(:,:,p)));      % FRET intensity of cell
                    acceptorInt(p,j)=sum(acceptorImage(maskCellFinal(:,:,p)));  % acceptor intensity of cell
                end
            end 
        ...
        elseif size(imageData2, 3) == numChannels                           % pre frame and no timeseries 
            ...
            for j=1:length(imageFiles)                      
                ...
                for p=1:size(maskCellFinal,3)
                    ...
                    donorInt(p,j)=sum(donorImage(maskCellFinal(:,:,p)));
                    FRETInt(p,j)=sum(FRETimage(maskCellFinal(:,:,p)));
                    acceptorInt(p,j)=sum(acceptorImage(maskCellFinal(:,:,p)));
                end
            end
            times = -300;                  
...
title('Acceptor')
saveas(CellImage, fullfile(folder,'CellImage.png'))
close(CellImage)   
...
saveas(FRETgraphs, fullfile(folder,'FRETgraphs.png'))
